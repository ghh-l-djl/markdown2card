...
- 集成 AI 智能重写模块，在导出后置动作中可选择调用 Gemini API 或本地 `agy -p`。Gemini 支持自定义基础地址、模型和 API Key；agy 支持配置可执行文件路径、代理地址和 `NO_PROXY`。两种方式共用可配置提示词模板与多语言输出，调用前和返回后都会清理图片引用；请求失败或返回空内容时使用已清理图片的源正文继续生成发布版文件。
...
Obsidian 当前文件
  ...
  -> 可选 YAML frontmatter 后处理 & 异步调用 AiManager.rewriteContent 分派 Gemini 或 agy 进行正文重写
...
- `src/aiManager.ts`：AI 重写服务分派模块，负责构造共享提示词，并选择 Gemini API 或本地 agy 提供方。
- `src/agyManager.ts`：本地 agy 子进程执行模块，通过参数数组调用可配置的 `agy -p`，注入代理环境并处理非零退出和空输出。
